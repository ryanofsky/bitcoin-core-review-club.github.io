...
* The PR introduces a **single command‑line front‑end** called **`bitcoin`**
  ...
  The wrapper therefore accomplishes the “side‑binaries + unified entry point”
...
2. Try a list of candidate paths for the target binary, in priority:
...
3. Call `util::ExecVp()` with each candidate, moving onto the next candidate
   ...
   if different error is returned or there is no next candidate.
...
* Functional test framework understands `BITCOIN_CMD="bitcoin -m"` so the
  entire suite can be driven through the new CLI without.
...
1. From [issue #30983](https://github.com/bitcoin/bitcoin/issues/30983), four
   ...
   *Which specific drawbacks of the* “side‑binaries without wrapper”
   *approach does this PR address?*

1. In `util::ExecVp()` (Windows branch) why is a **second** `std::vector`
   `escaped_args` needed instead of modifying `argv` in‑place?

1. Walk through the escaping algorithm for the argument
...
1. PR adds an exemption in `security-check.py` because the wrapper contains no
...
1. (Forward‑looking) Once `bitcoin-gui` actually spawns `bitcoin-node`
   automatically (after #10102 lands), what additional command‑line options
   or UX changes might the wrapper need?
