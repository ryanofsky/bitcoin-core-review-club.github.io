...
1. **Batch atomicity** What wallet invariants could be violated if the `WalletBatch` transaction inside `ExportWatchOnlyWallet` were not used (i.e. each record written individually)?
...
1. **Encrypted wallets** `exportwatchonlywallet` operates while the source wallet is locked.
   Walk through the code path that avoids requiring a decryption key, especially when copying `CWalletTx` records.
